...
- Selected controlled pilot package: `fixtures/controlled-pilot/webhook-review-notification.dry-run.json` and `fixtures/controlled-pilot/input-package.schema.json` cover the webhook review to HTTP notification dry-run input, explicit approval checkpoint, and focused coverage via `npm test -- test/controlled-pilot-package.test.ts`.
