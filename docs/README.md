...
- [Controlled Pilot Candidate Catalog](./controlled-pilot-candidate-catalog.md): Phase 6 comparison of manual approval plus notification, scheduled repository hygiene report, webhook intake to human review to HTTP notification, and Ensen-loop dry-run dispatch, with one selected owner-controlled dry-run pilot and explicit non-claims.
- Selected controlled pilot package: `fixtures/controlled-pilot/webhook-review-notification.dry-run.json` and `fixtures/controlled-pilot/input-package.schema.json` cover the webhook review to HTTP notification dry-run input, explicit approval checkpoint, repeatable fake-transport command `node dist/cli.js run-controlled-pilot fixtures/controlled-pilot/webhook-review-notification.dry-run.json <state-root> [audit-jsonl-path]`, and focused coverage via `npm test -- test/controlled-pilot-package.test.ts`.
